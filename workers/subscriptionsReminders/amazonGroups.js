...
const amazonGroupsReminders = require('../../libs/subscriptionsReminders/amazonGroups');
...
function worker (job, done) {
  ...
  amazonGroupsReminders.findAffectedGroups(habitrpgGroups, habitrpgUsers, null, moment.utc(), queue, baseUrl)
    ...
    .catch(err => { // The processing errored, crash the job and log the error
      console.log('Error while sending reminders for Amazon group subscriptions', util.inspect(err, false, null));
      done(err);
    });
}
